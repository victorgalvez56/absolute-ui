 ...
 *
 * The outer theme-aware gradient is what the glass primitives
 * composite against — Aurora / Obsidian show a dark photographic
 * radial, Frost / Sunset show a light warm one. The gradient is
 * the only thing that produces the backdrop-blur effect on
 * react-native-web; without it, every glass surface would read as
 * a flat tint.
 */
...
type Props = {
  theme: Theme;
  children: ReactNode;
  /** Hide the clock + island row (e.g. for full-bleed onboarding). */
...
/**
 * Per-theme background gradient. Intentionally different from the
 * Ladle default Backdrop so screens read as a "device" rather than
 * a story card.
 */
...
/**
 * Centers one or more PhoneFrame instances on a neutral backdrop so
 * individual stories don't each re-implement the layout.
 */
